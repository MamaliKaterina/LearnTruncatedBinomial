...
from learning_truncated_binomial_algos.learn_p_known_n_system \
import learn_p_known_n_system
...
for i in range(3):
  for j in range(3):
      ...
      #estimation
      p_est, samples =\
        learn_p_known_n_system(n, true_dist, S, a,\
...
fig.suptitle('Bin('+str(n)+', '+str(round(p,2))+')   ε='+str(epsilon),\
              y=0.98, size='xx-large', weight='black', color='white')
...
for i in range(3):
  for j in range(3):
      ...
      if j == 0:
          axs[i, j].set_ylabel('α='+str(round(a, 2)), size='x-large', color='white')

plt.xlim([230,370])
plt.ylim([-0.0001, 0.03])

labels = ['True Distr', 'Estimated Distr']
plt.figlegend(labels, fontsize='large', labelcolor='white')

plt.savefig('images/known_n_system_solution_testing.pdf')
plt.show()
